...
//  ".URL(conversationURL($data["conversation"]["conversationId"], $data["conversation"]["title"]), true)."	
	...
	public function handler_conversationController_renderScrubberBefore($sender, $data)
	{
		$controls = "<ul id='conversationShareControls'>
					...
					<li><a href='https://pinterest.com/pin/create/button/?url=".URL(conversationURL($data["conversation"]["conversationId"], $data["conversation"]["title"]), true)."&description=".$data["conversation"]["title"]."' target='_blank'><i class='icon-pinterest'></i><span>".T("Share on")." Pinterest</span></a></li>
